...
{
    [ComImport]
    [Guid(InterfaceID.IID_IDirectSoundCaptureBuffer8)]
    [InterfaceType(ComInterfaceType.InterfaceIsIUnknown)]
    public interface IDirectSoundCaptureBuffer8
    {
        [PreserveSig]
        uint GetCaps([In, Out] ref DSCBCAPS pDSCBCaps);
        ...
        [PreserveSig]
        uint GetFormat(out WAVEFORMATEX pwfxFormat, int dwSizeAllocated, out int pdwSizeWritten);

        [PreserveSig]
        uint GetStatus(out int pdwStatus);

        [PreserveSig]
        uint Initialize(IntPtr pDirectSoundCapture, ref DSCBUFFERDESC pcDSCBufferDesc);
        ...
        [PreserveSig]
        uint GetObjectInPath(ref GUID rguidObject, int dwIndex, ref GUID rguidInterface, out IntPtr ppObject);
